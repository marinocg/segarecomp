#!/usr/bin/env python3
"""Pinned-Musashi differential for synthetic brief PC-relative indexed PEA.

Cross-checks the 32-bit address the shared runtime EA helper computes and
pushes for `PEA (0x10,PC,D1.W)` (M68kEaMode::pc_index8, SEG-021-T011)
against the pinned local Musashi MC68000 core. Skips cleanly (exit 0) unless
SEGARECOMP_M68K_PC_INDEX8_PEA_MUSASHI_CHECKOUT points at the pinned checkout.

Mirrors `m68k_index8_pea_musashi_differential_test.py`'s own PEA/(d8,An,Xn)
differential and doc comment (direct single-operation C emission, not yet
the full C4 block-dispatch route -- see that file's doc comment for the
full explanation) for PEA's other newly admitted control-EA form.
"""
import os
import pathlib
import shutil
...
CHECKOUT_ENV = "SEGARECOMP_M68K_PC_INDEX8_PEA_MUSASHI_CHECKOUT"

# PEA (0x10,PC,D1.W) at 0x00000B00 -> 0x487B 0x1010. The operand's own
# `pc_base_address` is the address of its first extension word, 0x00000B02.
ORACLE_SOURCE = r'''#include <stdint.h>
#include <stdio.h>
#include <string.h>
#include "m68k.h"
static const uint8_t code[] = {0x48, 0x7b, 0x10, 0x10};
...
def main():
    ...
    if not configured:
        print("pc-index8-PEA pinned Musashi oracle unavailable; synthetic generated-C checks completed without "
              "differential claim")
        return
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        generated = checked([executable, "--emit-operation-c4-pc-indexed-pea"]).stdout
    ...
    print("compared synthetic brief PC-relative indexed PEA push result against pinned Musashi")
