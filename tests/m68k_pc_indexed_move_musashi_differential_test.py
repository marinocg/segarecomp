#!/usr/bin/env python3
"""Pinned-Musashi differential for synthetic brief PC-relative indexed MOVEA.

Cross-checks the effective address and loaded value the shared runtime EA
helper computes for `MOVEA.L (0x10,PC,D1.W),A2` (M68kEaMode::pc_index8,
SEG-007-T136) against the pinned local Musashi MC68000 core. Skips cleanly
(exit 0) unless SEGARECOMP_M68K_PC_INDEXED_MOVE_MUSASHI_CHECKOUT points at the
pinned checkout.
"""
import os
import pathlib
...
CHECKOUT_ENV = "SEGARECOMP_M68K_PC_INDEXED_MOVE_MUSASHI_CHECKOUT"

# MOVEA.L (0x10,PC,D1.W),A2 at 0x00000B00 -> 0x247B 0x1010. The extension
# word's own address is 0x00000B02, so the read target is
# 0x00000B02 + sign_extend(D1.W) + 0x10 -- a synthetic project-authored data
# region, never commercial-derived bytes.
ORACLE_SOURCE = r'''#include <stdint.h>
#include <stdio.h>
#include "m68k.h"
static const uint8_t code[] = {0x24, 0x7b, 0x10, 0x10};
...
static void run(uint32_t d1, uint32_t seed_value) {
  ...
  m68k_write_memory_32(UINT32_C(0x00FF0080), seed_value);
  (void)m68k_execute(64);
  printf("%08X\n", m68k_get_reg(NULL, M68K_REG_A2));
}
int main(void) { run(UINT32_C(0x00000004), UINT32_C(0x11223344)); run(UINT32_C(0x0000FFF6), UINT32_C(0xAABBCCDD)); return 0; }
'''
...
static void run(uint32_t d1, uint32_t seed_value) {
  ...
  r.work_ram[UINT32_C(0x0080)] = (uint8_t)(seed_value >> 24U);
  r.work_ram[UINT32_C(0x0081)] = (uint8_t)(seed_value >> 16U);
  r.work_ram[UINT32_C(0x0082)] = (uint8_t)(seed_value >> 8U);
  r.work_ram[UINT32_C(0x0083)] = (uint8_t)(seed_value);
...
def main():
    ...
    if not configured:
        print("PC-indexed-MOVEA pinned Musashi oracle unavailable; synthetic generated-C checks completed without differential claim")
        return
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        generated = checked([executable, "--emit-general-startup-runtime-c4-pc-indexed-move"]).stdout
    ...
    print("compared synthetic brief PC-relative indexed MOVEA effective address and loaded value against pinned Musashi")
