...
struct M68kMemoryEmissionContext {
  ...
  // SEG-022-T006: when true, a runtime-computed JMP/JSR target is checked against the one generated
  // final compiled-address authority `genesis_compiled_entry_lookup` (declared before every body) instead
  // of a site-local array. Exclusive with a per-site proven `indirect_candidate_targets` set.
